...
// Auto-detect protocol and fallback if needed
function getAPIBaseUrl() {
    // If the current page is HTTPS, try HTTPS first, then fallback to HTTP
...
function askQuestion() {
    ...
    fetchWithRetry(`${API_BASE_URL}/ask`, {
        method: 'POST',
        headers: {
    ...
    .catch(error => {
        ...
        removeMessage(loadingMessageId);
        addErrorMessage('Connection error. Please check your connection and try again. If the problem persists, contact support.');
    })
    .finally(() => {
        // Re-enable button
...
// Check API health when app initializes
document.addEventListener('DOMContentLoaded', function() {
    // Run health check after a short delay
    setTimeout(() => {
        checkAPIHealth().then(healthy => {
            if (!healthy) {
                console.warn('API may not be available. Some features might not work.');
            }
        });
    }, 1000);
});
